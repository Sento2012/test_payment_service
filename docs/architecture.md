...
- Бизнес-слой не знает про SQLAlchemy: `UnitOfWork.begin()` отдаёт непрозрачный
  ...
  (не `IntegrityError`). Конкретный тип (`AsyncSession`) известен только адаптеру —
  репозиторий приводит хэндл к нему на входе метода (единственное место, где это уместно).
  Смена ORM = переписать адаптеры в `infrastructure`/`repository`, домен не трогается.
