...
- **Сервис** = класс с одной ответственностью. Без фасад-прокси и фабрик: логика прямо
  в классе, зависимости инжектятся через конструктор в DI-контейнере
  (`src/di/container.py`, без сторонних библиотек), `get_container()` — singleton на
  процесс.
