...
class PaymentExecutor:
    ...
    ключу payment.id результат сохраняется в стор; повторный вызов с тем же ключом
    возвращает сохранённый результат БЕЗ нового списания (защита от двойного charge
    при краше между списанием и commit'ом, в т.ч. между подами)."""

    def __init__(
        self,
        provider_execute_plugins: list[ProviderExecutePluginInterface],
        idempotency_store_service: PaymentProviderIdempotencyStoreRepositoryService,
    ) -> None:
        self._provider_execute_plugins = provider_execute_plugins
        self._idempotency_store_service = idempotency_store_service
    ...
    async def execute_payment(
        ...
        existing = await self._idempotency_store_service.find_record(
        ...
        stored = await self._idempotency_store_service.get_or_create_record(
            IdempotencyRecordDraftTransfer(
